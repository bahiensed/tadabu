...
const Multiplication = () => {
  ...
  const multiplier = parseInt(pathname.split("/").filter(Boolean).pop() || "1", 10);
      ...
      toast({
        ...
        description: (
          <pre className="mt-2 w-[320px] text-2xl font-semibold rounded-md bg-green-500 p-4">
            <code className="text-white">{randomNumber} x {multiplier} = {randomNumber * multiplier}</code>
          </pre>
        ),
      })
              ...
              <FormItem>
                <FormControl>
                  <div className="flex flex-row items-center gap-4">
                    {randomNumber !== null && (
                      <span className="text-6xl text-nowrap">{randomNumber} x {multiplier} =</span>
                    )}
